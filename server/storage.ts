...
export class SQLiteStorage implements IStorage {
  ...
  // ─── Salaries ───────────────────────────────────────────────────────────────
  getSalaries(masterId?: number, from?: string, to?: string) {
    let all = db.select().from(salaries).orderBy(desc(salaries.date)).all();
    if (masterId) all = all.filter(s => s.masterId === masterId);
    if (from) all = all.filter(s => s.date >= from);
    if (to)   all = all.filter(s => s.date <= to);
    return all;
  }
  getSalaryById(id: number) {
    return db.select().from(salaries).where(eq(salaries.id, id)).get();
  }
  createSalary(data: InsertSalary) {
    return db.insert(salaries).values(data).returning().get();
  }
  updateSalary(id: number, data: Partial<InsertSalary>) {
    return db.update(salaries).set(data).where(eq(salaries.id, id)).returning().get();
  }
  deleteSalary(id: number) {
    db.delete(salaries).where(eq(salaries.id, id)).run();
  }
  getSalaryStats(masterId?: number, from?: string, to?: string) {
    let all = db.select().from(salaries).all();
    if (masterId) all = all.filter(s => s.masterId === masterId);
    if (from) all = all.filter(s => s.date >= from);
    if (to)   all = all.filter(s => s.date <= to);
    const total = all.reduce((sum, s) => {
      if (s.type === 'penalty') return sum - s.amount;
      return sum + s.amount;
    }, 0);
    const paid = all.filter(s => s.paid).reduce((sum, s) => {
      if (s.type === 'penalty') return sum - s.amount;
      return sum + s.amount;
    }, 0);
