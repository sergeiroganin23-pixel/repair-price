...
export function registerRoutes(httpServer: Server, app: Express) {
  ...
  app.get("/api/salaries", authenticateToken, (req: AuthRequest, res: Response) => {
    ...
    res.json(storage.getSalaries(masterId ? parseInt(masterId) : undefined, from, to));
  });

  app.get("/api/salaries/stats", authenticateToken, (req: AuthRequest, res: Response) => {
    const { masterId, from, to } = req.query as { masterId?: string; from?: string; to?: string };
    res.json(storage.getSalaryStats(masterId ? parseInt(masterId) : undefined, from, to));
  });

  app.post("/api/salaries", authenticateToken, requireAdmin, (req: AuthRequest, res: Response) => {
    const { masterId, masterName, type, amount, description, repairId, period, paymentMethod, paid, date } = req.body;
    if (!masterId || !masterName || !amount) {
      return res.status(400).json({ error: "Мастер, имя и сумма обязательны" });
    }
    const now = new Date().toISOString();
    const salary = storage.createSalary({
  ...
  app.put("/api/salaries/:id", authenticateToken, requireAdmin, (req: AuthRequest, res: Response) => {
    const result = storage.updateSalary(parseInt(req.params.id), req.body);
